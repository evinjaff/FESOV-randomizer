...
7. Unfortunately, you will still have to use NTR cheats to promote classes. Check the Reclassing using romfs section to see how.
...
This allows all Amiibo-exclusive classes to be options. Amiibo-exclusive classes were the classes of the illusory fighters that can be summoned via scanning Amiibos such as Lucina, Roy, and Robin. These classes are untiered and possilbe randomizations at every level if enabled. This is by far the coolest option, since the characters models classed into these look a lot like how the Lodestar, Great Lord, Ranger, and Tactitian classes were in *Fates*! So, this means that you could be classed as Lucina, or Marth if you so pleased. **It is highly reccomended that you edit the various Amiibo class class strengths in paragon to be a value between 5 and 8 so that the Amiibo class characters don't steal all the XP** (default class strength is 9 and will make the early game extremely difficult if you decide to keep it that way). In total, this allows for these classes to be used:
